 ...
 // const dateRange = useMemo(() => ({ from: "2024-01-01", to: today }), [today]);
  ...
  const saveTimes = async () => {
    ...
    const check = (label: string, v: string, opts?: { ovOut?: boolean }) => {
     ...
     check('AM In', editTimes.amIn); check('AM Out', editTimes.amOut); 
     check('PM In', editTimes.pmIn); check('PM Out', editTimes.pmOut); 
     check('OV In', editTimes.ovIn); check('OV Out', editTimes.ovOut, { ovOut: true });

    if (invalids.length) { setSaving(false); setSaveError(invalids.join('; ')); return; }

    // Xây payload times theo code có trong sessions
    const times: any = {};
    if (editTimes.amIn || editTimes.amOut) times["AM"] = withPair(editTimes.amIn, editTimes.amOut);
    if (editTimes.pmIn || editTimes.pmOut) times["PM"] = withPair(editTimes.pmIn, editTimes.pmOut);
    if (editTimes.ovIn || editTimes.ovOut) times["OV"] = withPair(editTimes.ovIn, editTimes.ovOut);
                    ...
                    {dailyRows.map((row) => (
                      <tr key={row.dateKey} className="hover:bg-blue-50/40">
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 flex items-center gap-2">
                          {getDayNameFromDate(row.dateKey)}
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 flex items-center gap-2">
                          {row.dateKey}
                          {row.isManualEdit && (
...
function isEmptyEdits(
  ...
  const wantAM =  (e.amIn || e.amOut);
  const wantPM =  (e.pmIn || e.pmOut);
  const wantOV =  (e.ovIn || e.ovOut);  
